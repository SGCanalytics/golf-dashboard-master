...
def coachs_corner_tab(cc):
    ...
    if gir_flags:
        st.markdown(
            ...
            f'margin-bottom:0.5rem;">Approach (GIR &lt; 50%)</p>',
            unsafe_allow_html=True,
        )
        gir_cols = st.columns(len(gir_flags))
        for col, gf in zip(gir_cols, gir_flags):
            with col:
                premium_stat_card(
                    gf["bucket"], format_pct(gf["gir_pct"]),
                    "GIR", sentiment="negative",
                )
    else:
        st.markdown(
            f'<p style="font-family:{FONT_BODY};font-size:0.8rem;color:{SLATE};'
            f'font-style:italic;">No GIR red flags.</p>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        f'<p style="font-family:{FONT_BODY};font-size:0.75rem;font-weight:600;'
        f'color:{SLATE};text-transform:uppercase;letter-spacing:0.06em;'
        f'margin:1rem 0 0.5rem 0;">Short Game (Inside 8 ft)</p>',
        unsafe_allow_html=True,
    )
    ...
    with sg_c1:
        premium_stat_card(
            "Fairway / Rough",
            format_pct(sgf["inside8_fr_pct"]),
            "inside 8 ft",
            sentiment=pct_sentiment_above(sgf["inside8_fr_pct"], "pct_inside_8ft"),
        )
    with sg_c2:
        s = "positive" if sgf["inside8_sand_pct"] >= 40 else "negative"
        premium_stat_card(
            "Sand",
            format_pct(sgf["inside8_sand_pct"]),
            "inside 8 ft",
            sentiment=s,
        )
    ...
    st.markdown(
        f'<p style="font-family:{FONT_BODY};font-size:0.75rem;font-weight:600;'
        f'color:{SLATE};text-transform:uppercase;letter-spacing:0.06em;'
        f'margin:1rem 0 0.5rem 0;">Putting</p>',
        unsafe_allow_html=True,
    )
    pf = cc["putting_flags"]
    pf_c1, pf_c2, pf_c3, pf_c4 = st.columns(4)
    with pf_c1:
        s = "positive" if pf["make_45_pct"] >= 60 else "negative"
        premium_stat_card("Make % (4\u20135 ft)", format_pct(pf["make_45_pct"]),
                          sentiment=s)
    with pf_c2:
        premium_stat_card("SG (5\u201310 ft)", format_sg(pf["sg_510"]),
                          sentiment=sg_sentiment(pf["sg_510"]))
    with pf_c3:
        s = pct_sentiment_below(pf["lag_miss_pct"], "pct_lag_miss")
        premium_stat_card("Lag Miss %", format_pct(pf["lag_miss_pct"]),
                          ">5 ft from 30+", sentiment=s)
    with pf_c4:
        s = "negative" if pf["three_putts_inside_20"] > 0 else "positive"
        premium_stat_card("3-Putts <20 ft", str(pf["three_putts_inside_20"]),
                          sentiment=s)
